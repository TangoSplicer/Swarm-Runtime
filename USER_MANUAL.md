# Swarm Runtime: Developer User Manual (v1.0.0)

## 1. Starting the Mesh & Cryptographic Identity
Every node dynamically generates a permanent cryptographic identity (`.swarm_identity`) on its first boot using the cross-platform `rand::rngs::OsRng`.
