...
Android imposes strict memory constraints. Add these to your `~/.bashrc`:
`export RUST_MIN_STACK=8388608`
`export CARGO_PROFILE_DEV_DEBUG=0`

## 3. Build & Execution Protocol
Always restrict Cargo to a single parallel compilation job to prevent the Android Out-Of-Memory (OOM) killer from terminating the compiler:
