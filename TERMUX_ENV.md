...
To ensure exact reproducibility of the Swarm Runtime (v0.24.0) on Android, this document details the exact Termux ecosystem parameters, packages, and constraints required.
...
​⚠️ CRITICAL FOR LOCAL TESTING: If you are running multiple Workers on the same Termux instance, you cannot share the same host directory.
You must physically isolate the execution paths so they generate their own .state and .swarm_identity files without read/write collisions:                                                mkdir worker1_dir && mkdir worker2_dir
